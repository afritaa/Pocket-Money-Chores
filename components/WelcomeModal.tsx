...
const WelcomeModal: React.FC<WelcomeModalProps> = ({ isOpen, onSave }) => {
  ...
  const [isPasscodeSkipped, setIsPasscodeSkipped] = useState(false);
  ...
  const handleNextStep = useCallback(() => {
    ...
    setStep(s => s + 1);
  }, [step, name, passcode, confirmPasscode, isPasscodeSkipped]);
  ...
  const handleSaveAll = useCallback(() => {
    ...
    onSave({
      ...
      image,
      payDayConfig,
      passcode: isPasscodeSkipped || !passcode ? null : passcode
    });
  }, [step, isPasscodeSkipped, passcode, confirmPasscode, onSave, name, image, payDayConfig, playButtonClick]);
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    if (event.key === 'Enter') {
      ...
      const primaryButton = focusable.find(
        b => b.textContent?.includes('Next') || b.textContent?.includes('Finish Setup')
      );
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
           <div className="animate-fade-in">
             <h3 className="text-xl font-bold mb-1">Let's set up your first child</h3>
             <p className="text-[var(--text-secondary)] mb-6">You can add more children later.</p>
             <div className="flex flex-col items-center space-y-4 mb-6">
                ...
                <label htmlFor="welcome-image-upload" className="cursor-pointer px-4 py-2 rounded-lg text-sm text-[var(--text-primary)] bg-[var(--bg-tertiary)] hover:opacity-80 font-semibold border border-[var(--border-secondary)] transition-all">Upload Picture</label>
                <input id="welcome-image-upload" type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
              </div>
              <div>
      ...
      case 3:
        return (
          <div className="animate-fade-in">
            <h3 className="text-xl font-bold mb-1">Parent Passcode (Optional)</h3>
            <p className="text-[var(--text-secondary)] mb-6">Secure Parent Mode to prevent changes to chores and settings.</p>
            {!isPasscodeSkipped ? (
              <div className="space-y-4 animate-fade-in-fast">
                <div>
                  <label htmlFor="welcome-passcode" className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Set 4-Digit Passcode</label>
                  <input id="welcome-passcode" type="password" inputMode="numeric" maxLength={4} value={passcode} onChange={e => setPasscode(e.target.value.replace(/\D/g, ''))} placeholder="••••" className="w-full px-4 py-3 text-center tracking-[1em] text-2xl bg-[var(--bg-tertiary)] border-[var(--border-secondary)] border rounded-lg focus:ring-2 focus:ring-[var(--accent-primary)] transition-all"/>
                </div>
                {passcode && (
                 ...
                 <button type="button" onClick={() => setIsPasscodeSkipped(true)} className="text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--accent-primary)] hover:underline">
                  Skip for now
                 </button>
              </div>
            ) : (
                <div className="text-center p-8 bg-[var(--bg-tertiary)] rounded-lg border border-[var(--border-primary)]">
                    <p className="font-semibold text-[var(--text-primary)]">Passcode skipped.</p>
                    <p className="text-sm text-[var(--text-secondary)] mt-2">You can set one up later in the Parent Settings.</p>
                    <button type="button" onClick={() => setIsPasscodeSkipped(false)} className="mt-4 text-sm font-semibold text-[var(--accent-primary)] hover:underline">
  ...
  return (
    ...
    >
      ...
      >
        <div className="mb-6">
          <h2 className="text-2xl font-bold mb-4 text-center">Welcome to Pocket Money Chores!</h2>
        ...
        <div className="flex justify-between items-center mt-8 pt-4 border-t border-[var(--border-primary)]">
            <button 
                ...
                className={`px-6 py-2 rounded-lg text-[var(--text-primary)] bg-[var(--bg-tertiary)] hover:opacity-80 border border-[var(--border-secondary)] font-semibold transition-colors ${step === 1 ? 'opacity-0 pointer-events-none' : ''}`}
            >
                Back
            </button>
            {step < totalSteps ? (
                ...
                <button onClick={handleSaveAll} className="px-6 py-2 rounded-lg text-[var(--success-text)] bg-[var(--success)] hover:opacity-80 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-px transition-all">Finish Setup</button>
            )}
        </div>
      </div>
      <style>{`
