...
const ProfileSelector: React.FC<ProfileSelectorProps> = ({ profiles, onSelectProfile, onAttemptSwitchToParentMode, lastActiveProfileId }) => {
    ...
    return (
        <div className="fixed inset-0 bg-[var(--bg-primary)] z-[100] flex flex-col items-center justify-center p-8 transition-opacity duration-500 animate-fade-in">
            <h1 className="text-4xl font-bold text-[var(--text-primary)] mb-12 drop-shadow-sm">Pocket Money Chores.</h1>
            ...
            <button
                ...
                className="mt-20 flex items-center gap-3 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors py-2 px-4 rounded-lg hover:bg-white/10"
            >
                <LockClosedIcon className="w-6 h-6" />
                <span className="text-lg font-semibold">Parent Mode</span>
            </button>
            <style>{`
                @keyframes fade-in { 
                  from { opacity: 0; } 
