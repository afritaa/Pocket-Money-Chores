...
type Step = 'welcome' | 'child_needs_parent' | 'parent_great';
...
const DeviceSetupModal: React.FC<DeviceSetupModalProps> = ({ hasProfiles, onSetParentDevice, onSetChildDevice, onAddFirstChild }) => {
  ...
  const handleAddChildAndSetDevice = () => {
    onSetParentDevice();
    onAddFirstChild();
  };
  ...
  const cardClasses = "bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-3xl p-8 md:p-12 text-center text-[var(--text-primary)] max-w-2xl w-full shadow-2xl";
  ...
  if (step === 'parent_great') {
    return (
      <div className={`${cardClasses} animate-fade-in-fast`}>
        <h2 className="text-4xl md:text-5xl font-bold mb-4">Great!</h2>
        <div className="text-lg md:text-xl text-[var(--text-secondary)] mb-10 max-w-md mx-auto">
            <p>
                You are on the way to creating an incentivised pocket money system that works for you, your child, and your family!
            </p>
            <p className="mt-4">
                But first...
            </p>
        ...
        <button
          onClick={handleAddChildAndSetDevice}
          className="bg-[var(--accent-primary)] text-[var(--accent-primary-text)] hover:bg-[var(--accent-secondary)] transition-colors font-bold text-lg py-4 px-10 rounded-xl"
        >
