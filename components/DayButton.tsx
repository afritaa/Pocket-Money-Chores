...
const DayButton: React.FC<DayButtonProps> = ({ day, isAssigned, isCompleted, isCashedOut, isPendingCashOut, isPendingAcceptance, isToday, isPast, onClick, isKidsMode, isPendingApproval, isBonus }) => {
  ...
  let dynamicClasses = '';
  const isDisabled = !isAssigned || isBonus || (isKidsMode && (isCashedOut || isPendingCashOut || isPendingAcceptance));
  ...
  return (
    ...
    >
      {isBonus ? <span className="text-sm sm:text-base font-bold">$</span> : isCashedOut ? <CoinIcon className="w-5 h-5 sm:w-6 sm:h-6"/> : isPendingCashOut || isPendingAcceptance ? <HourglassIcon className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500" /> : isCompleted ? <CheckIcon className="w-4 h-4 sm:w-5 sm:h-5"/> : isPendingApproval ? <ExclamationIcon className="w-4 h-4 sm:w-5 sm:h-5" /> : DAY_SHORT_NAMES[day]}
    </button>
  );
};
